...
draw.text((50, 60), '排序修复：长度优先 + 词频其次', fill='#666', font=small_font)
...
# 候选词窗口1
candidate_box1 = (50, 150, 430, 280)
draw.rectangle(candidate_box1, fill='white', outline='#ddd', width=1)
draw.rectangle([50, 150, 430, 190], fill='#667eea')
draw.text((80, 168), input_pinyin1, fill='white', font=content_font)
draw.text((340, 168), '手动分词', fill='white', font=small_font)
...
for idx, (word, freq) in enumerate(candidates1):
    x_pos = x_start1 + idx * 100
    
    if idx == 0:
        draw.text((x_pos, y_offset1), '1.', fill='#667eea', font=number_font)
        word_font = ImageFont.truetype(font_path, 30)
        draw.text((x_pos + 30, y_offset1 - 5), word, fill='#333', font=word_font)
        draw.rectangle([x_pos - 5, y_offset1 - 10, x_pos + 110, y_offset + 45], outline='#667eea', width=2)
    else:
        draw.text((x_pos, y_offset1), f'{idx+1}.', fill='#666', font=number_font)
        draw.text((x_pos + 30, y_offset1 - 5), word, fill='#666', font=content_font)

# 测试2: dongnandaxue
input_pinyin2 = "dongnandaxue"
candidates2 = manager.generate_candidates(input_pinyin2)

draw.text((470, 100), '测试2: dongnandaxue (自动分词)', fill='#333', font=content_font)
draw.text((470, 120), f'分词: {engine.smart_split_pinyin(input_pinyin2)}', fill='#666', font=small_font)

# 候选词窗口2
candidate_box2 = (470, 150, 850, 280)
draw.rectangle(candidate_box2, fill='white', outline='#ddd', width=1)
draw.rectangle([470, 150, 850, 190], fill='#667eea')
draw.text((500, 168), input_pinyin2, fill='white', font=content_font)
draw.text((760, 168), '自动分词', fill='white', font=small_font)
...
for idx, (word, freq) in enumerate(candidates2):
    x_pos = x_start2 + idx * 100
    
    if idx == 0:
        draw.text((x_pos, y_offset2), '1.', fill='#667eea', font=number_font)
        word_font = ImageFont.truetype(font_path, 30)
        draw.text((x_pos + 30, y_offset2 - 5), word, fill='#333', font=word_font)
        draw.rectangle([x_pos - 5, y_offset2 - 10, x_pos + 110, y_offset + 45], outline='#667eea', width=2)
    else:
        draw.text((x_pos, y_offset2), f'{idx+1}.', fill='#666', font=number_font)
        draw.text((x_pos + 30, y_offset2 - 5), word, fill='#666', font=content_font)

# 状态栏
status_bar = (50, 450, 850, 480)
draw.rectangle(status_bar, fill='#f9f9f9')
draw.text((70, 465), '🟢 排序已修复', fill='#666', font=small_font)
draw.text((200, 465), f'输入1: {input_pinyin1}, 候选词: {len(candidates1)}', fill='#666', font=small_font)
draw.text((550, 465), f'输入2: {input_pinyin2}, 候选词: {len(candidates2)}', fill='#666', font=small_font)
draw.text((700, 465), '优先级: 长度优先 + 词频其次', fill='#666', font=small_font)
